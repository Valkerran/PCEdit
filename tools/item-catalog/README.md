...
page. It provides presentation, logistics eligibility, deprecation, and game-version
metadata for each known item id. The catalog is **app-only** — it is never read from
or written to a save file.
...
Holds the curated `GId -> (displayName, category)` table, logistics membership,
and the category table (`category -> displayName, icon`). Edit the tables at the top
and re-run. Every item's category must exist in `CATEGORIES` (the script asserts this, and
`ItemCatalogTests.EmbeddedCatalog_ParsesAndEveryItemCategoryIsDefined` re-checks
it at build time).

Every generated item has `canDemand` and `canSupply` flags, an explicit `deprecated`
flag, and nullable `addedIn` / `deprecatedIn` game-version strings. Deprecated items
remain resolvable for existing saves but are omitted from logistics pickers. Until an
authoritative version is known, leave the corresponding version field null.
...
It lists every `WorldObject.gId` and `unlockedGroups` entry absent from `ItemCatalog.json`, plus
every `demandGrps` or `supplyGrps` id without the corresponding catalog capability. Occurrence
counts put the ids worth naming first at the top. Curate the output into `ITEMS` and
`LOGISTICS_IDS`, then regenerate the catalog.
