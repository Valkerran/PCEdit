...
public sealed class PlanetHashTests
{
    ...
    [Fact]
    public void Of_MatchesThePlanetIntOnARealSaveFilesWorldObjects()
    {
        ...
        var primeHash = PlanetHash.Of(save.Metadata.PlanetId);
        var placed = save.WorldObjects.Where(w => w.Planet is not null).ToList();

        Assert.NotEmpty(placed);
        Assert.All(placed, w => Assert.Equal(primeHash, w.Planet));
    }
}
